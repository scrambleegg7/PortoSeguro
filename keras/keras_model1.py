...
from PortoSeguro.keras.callBClass import roc_auc_callback
...
DATA_TRAIN_PATH = env['train']
DATA_TEST_PATH = env['test']
...
#
# should be set to appropriate folds / run
#
...
#
# patience / batchsize is determined ....
#
patience = 10
batchsize = 128
...
# explained above.
        ...
        callbacks = [
            ...
            CSVLogger('output/keras-5fold-run-01-v1-epochs.log', separator=',', append=False),
            ModelCheckpoint(
                    'output/keras-5fold-run-01-v1-fold-' + str('%02d' % (i + 1)) + '-run-' + str('%02d' % (run + 1)) + '.check',
...
# validation and test data, and create a summary of parameters for each run.
        ...
        del nnet
        nnet = load_model('output/keras-5fold-run-01-v1-fold-' + str('%02d' % (i + 1)) + '-run-' + str('%02d' % (run + 1)) + '.check')
...
sub_file = 'output/train_5fold-keras-run-01-v1-oof_' + str(score) + '_' + str(now.strftime('%Y-%m-%d-%H-%M')) + '.csv'
...
sub_file = 'output/submission_5fold-average-keras-run-01-v1_' + str(score) + '_' + str(now.strftime('%Y-%m-%d-%H-%M')) + '.csv'
